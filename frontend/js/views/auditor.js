    ...
    appInstance.contentDiv.innerHTML = `
        ...
        <div class="glass-card rounded-3xl border border-slate-200/80 shadow-sm w-full mt-6 overflow-hidden animate-fade-in-up bg-white/50">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-6 pb-4 border-b border-slate-100">
                ...
                <div class="text-xs font-bold text-[#0F766E] bg-teal-50 px-3 py-1 rounded-full border border-teal-100" id="table-total-count">แสดง 0 รายการ</div>
            </div>
    ...
    const updateDashboard = () => {
        ...
        if (totalPages <= 1) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                if (i === tablePage) {
                    pagesHtml += `<button data-page="${i}" class="btn-table-page px-2.5 py-1 text-xs font-bold rounded-lg bg-[#0F766E] text-white shadow-sm">${i}</button>`;
                } else {
