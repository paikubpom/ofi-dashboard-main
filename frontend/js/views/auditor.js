...
export function renderAuditorView(appInstance, chartSettings = {}, currentRole = 'auditor') {
    ...
    appInstance.contentDiv.innerHTML = `
        ...
        <div id="dynamic-auditor-grid" class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6"></div>
        ...
        <div class="glass-card rounded-3xl border border-slate-200/80 shadow-sm w-full mt-6 overflow-hidden animate-fade-in-up bg-white/50">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs sm:text-[13px] table-auto">
                    <thead>
                        <tr class="bg-[#0F766E] text-white font-bold text-[11px] uppercase tracking-wider divide-x divide-white/10">
    ...
    const updateDashboard = () => {
        ...
        tableBody.innerHTML = currentOfis.map(o => {
            ...
            return `
                <tr class="hover:bg-teal-50/40 transition-colors group cursor-pointer animate-row-enter" data-id="${rId}">
                    ...
                    <td class="py-3 px-4 text-center font-bold text-[#0F766E] whitespace-nowrap">${rLvl}</td>
                </tr>
            `;
        }).join('');
        ...
        const showOfisPopup = () => {
            ...
            const renderPopupRows = () => {
                ...
                const tableRowsHtml = filtered.map(o => {
                    ...
                    return `
                        <tr class="hover:bg-teal-50/40 transition-colors group cursor-pointer border-b border-slate-100" data-id="${rId}">
                            <td class="py-3 px-4 font-bold text-slate-700 text-center whitespace-nowrap">${rMod}</td>
                            <td class="py-3 px-4 text-slate-600 max-w-[400px] lg:max-w-[600px] xl:max-w-[800px] truncate" title="${rTopic}">${rTopic}</td>
                            <td class="py-3 px-4 text-slate-500 font-medium whitespace-nowrap">${rOwner}</td>
                            <td class="py-3 px-4 text-center">
            ...
            const popupContentHtml = `
                <div class="space-y-4">
                    ...
                    <div class="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/60 flex flex-wrap gap-3 items-center">
                        ...
                        <div class="ml-auto text-xs font-bold text-[#0F766E] bg-teal-50 px-3 py-1 rounded-full border border-teal-100 whitespace-nowrap" id="popup-total-count-badge">แสดง ${filteredRecords.length} รายการ</div>
                    </div>
                    ...
                    <div class="overflow-hidden border border-slate-200/80 rounded-2xl shadow-sm bg-white">
                        <div class="overflow-x-auto max-h-[72vh] overflow-y-auto" style="overscroll-behavior: contain;">
                            <table class="w-full text-left border-collapse text-xs sm:text-[13px] table-auto">
                                <thead>
                                    <tr class="bg-[#0F766E] text-white font-bold text-[11px] uppercase tracking-wider divide-x divide-white/10 sticky top-0 z-10">
